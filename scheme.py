...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        bind = expressions.second
        env.define(target, scheme_eval(bind.first, env))
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        name = LambdaProcedure(target.second, expressions.second, env)
...
class Promise:
    ...
    def evaluate(self):
        if self.expression is not None:
            self.value = scheme_eval(self.expression,
                                     self.env.make_child_frame(nil, nil))
            self.expression = None
        return self.value

    def __str__(self):
        return '#[promise ({0}forced)]'.format('not ' if self.expression is
                                               not None else '')
...
def create_global_frame():
    ...
    env = Frame(None)
    env.define('eval', PrimitiveProcedure(scheme_eval, True, 'eval'))
    env.define('apply', PrimitiveProcedure(scheme_apply, True, 'apply'))
    env.define('load', PrimitiveProcedure(scheme_load, True, 'load'))
    env.define('call/cc', PrimitiveProcedure(scheme_call_cc, True, 'call/cc'))
